	...
	public class ClientSocket : CommonSocket
	{
		...
		internal bool StartConnection(string address, ushort port, bool peerToPeer)
		{
			try
			{
				...
				if (PeerToPeer)
				{
					_socket = SteamNetworkingSockets.ConnectP2P(ref identityRemote, port, steamNetworkingConfig.Length, steamNetworkingConfig);
				}
				else
				{
					...
					_socket = SteamNetworkingSockets.ConnectByIPAddress(ref address2, 0, steamNetworkingConfig);
				}
			}
			catch
			{
