	...
	public class ServerSocket : CommonSocket
	{
		...
		internal bool StartConnection(string address, ushort port, int maximumClients, bool peerToPeer)
		{
			try
			{
				...
				if (PeerToPeer)
				{
					_socket = SteamNetworkingSockets.CreateListenSocketP2P(port, steamNetworkingConfig.Length, steamNetworkingConfig);
				}
				else
				{
					...
					_socket = SteamNetworkingSockets.CreateListenSocketIP(ref localAddress, 0, steamNetworkingConfig);
				}
			}
			catch
			{
		...
		{
			if (args.m_info.m_hListenSocket != _socket)
			{
				return;
			}
