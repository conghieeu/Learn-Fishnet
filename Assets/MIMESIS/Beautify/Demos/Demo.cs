...
namespace Beautify.Demos
{
	public class Demo : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			{
				float value = BeautifySettings.settings.anamorphicFlaresIntensity.value;
				BeautifySettings.settings.anamorphicFlaresIntensity.Override((value > 0f) ? 0f : 1f);
			}
			if (Input.GetKeyDown(KeyCode.Alpha9))
			{
				float value2 = BeautifySettings.settings.blurIntensity.value;
				BeautifySettings.settings.blurIntensity.Override((value2 > 0f) ? 0f : 1f);
			}
		}
