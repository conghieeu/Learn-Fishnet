...
namespace ReluProtocol
{
	...
	public class PlayerInfo : OtherCreatureInfo, IMemoryPackable<PlayerInfo>, IMemoryPackFormatterRegister
	{
		[Preserve]
		private sealed class PlayerInfoFormatter : MemoryPackFormatter<PlayerInfo>
		{
			[Preserve]
			public override void Serialize<TBufferWriter>(ref MemoryPackWriter<TBufferWriter> writer, ref PlayerInfo value)
			{
		...
		[Preserve]
		public static void Serialize<TBufferWriter>(ref MemoryPackWriter<TBufferWriter> writer, ref PlayerInfo? value) where TBufferWriter : class, IBufferWriter<byte>
		{
			if (value == null)
			{
			...
			writer.WriteUnmanagedWithObjectHeader<ActorType, int, int>(18, value.actorType, value.actorID, value.masterID);
			...
			writer.WriteUnmanaged<ulong, int, bool>(value.steamID, value.currentInventorySlot, value.firstEnterMap);
		}
		...
		public static void Deserialize(ref MemoryPackReader reader, ref PlayerInfo? value)
		{
			...
			if (memberCount == 18)
			{
				if (value != null)
				{
					...
					value16 = value.steamID;
					value17 = value.currentInventorySlot;
					value18 = value.firstEnterMap;
					...
					reader.ReadUnmanaged<ulong>(out value16);
				...
				reader.ReadUnmanaged<ulong, int, bool>(out value16, out value17, out value18);
			}
			else
			{
				if (memberCount > 18)
				{
					MemoryPackSerializationException.ThrowInvalidPropertyCount(typeof(PlayerInfo), 18, memberCount);
					return;
				}
				if (value == null)
				{
			...
			value = new PlayerInfo
			{
				...
				inventories = value15,
				steamID = value16,
				currentInventorySlot = value17,
				firstEnterMap = value18
			};
			return;
			IL_03bb:
			...
			value.steamID = value16;
			value.currentInventorySlot = value17;
			value.firstEnterMap = value18;
		}
	}
}
