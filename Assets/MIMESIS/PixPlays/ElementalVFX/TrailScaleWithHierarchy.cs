...
namespace PixPlays.ElementalVFX
{
	...
	public class TrailScaleWithHierarchy : MonoBehaviour
	{
		...
		private void Update()
		{
			_trailRenderer.widthMultiplier = GetWorldScale(base.transform, _initialWidth);
			_trailRenderer.time = _initialTrailTime * _trailRenderer.widthMultiplier;
		}

		private float GetWorldScale(Transform obj, float scale = 1f)
		{
			float num = obj.localScale.x * scale;
			if (obj.parent != null)
			{
				return GetWorldScale(obj.parent, num);
			}
			return num;
		}
	}
