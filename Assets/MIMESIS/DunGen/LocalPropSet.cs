...
namespace DunGen
{
	...
	public class LocalPropSet : RandomProp
	{
		...
		public override void Process(RandomStream randomStream, Tile tile, ref List<GameObject> spawnedObjects)
		{
			...
			for (int i = 0; i < num; i++)
			{
				GameObjectChance random = gameObjectChanceTable.GetRandom(randomStream, tile.Placement.IsOnMainPath, tile.Placement.NormalizedDepth, null, allowImmediateRepeats: true, removeFromTable: true, allowNullSelection: true);
