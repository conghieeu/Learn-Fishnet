	...
	public sealed class DungeonProxy
	{
		...
		internal void ConnectOverlappingDoorways(float globalChance, DungeonFlow dungeonFlow, RandomStream randomStream)
		{
			...
			foreach (DoorwayProxy item in enumerable)
			{
				...
				{
					if (item.Used || item2.Used || item == item2 || item.TileProxy == item2.TileProxy)
					{
						continue;
					}
					ProposedConnection connection = new ProposedConnection(this, item.TileProxy, item2.TileProxy, item, item2);
					if (!dungeonFlow.CanDoorwaysConnect(connection) || (item.Position - item2.Position).sqrMagnitude >= 1E-05f)
					{
						continue;
					}
