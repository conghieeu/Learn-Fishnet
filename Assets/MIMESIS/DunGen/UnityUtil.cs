...
namespace DunGen
{
	public static class UnityUtil
	{
		...
		public static Bounds CalculateObjectBounds(GameObject obj, bool includeInactive, bool ignoreSpriteRenderers, bool ignoreTriggerColliders = true)
		{
			...
			{
				if (!(collider is TerrainCollider) && (!ignoreTriggerColliders || !collider.isTrigger))
				{
					if (flag)
					{
			...
			Terrain[] componentsInChildren4 = obj.GetComponentsInChildren<Terrain>(includeInactive);
			foreach (Terrain terrain in componentsInChildren4)
			{
				Bounds bounds = terrain.terrainData.bounds;
				bounds.center += terrain.gameObject.transform.position;
				if (flag)
				{
		...
		{
			float num = Mathf.Abs(Vector3.Dot(direction, new Vector3(1f, 0f, 0f)));
			float num2 = Mathf.Abs(Vector3.Dot(direction, new Vector3(0f, 1f, 0f)));
			float num3 = Mathf.Abs(Vector3.Dot(direction, new Vector3(0f, 0f, 1f)));
			if (num > 0.99f && num2 < 0.01f && num3 < 0.01f)
			{
				return true;
			}
			if (num2 > 0.99f && num < 0.01f && num3 < 0.01f)
			{
				return true;
			}
			if (num3 > 0.99f && num < 0.01f && num2 < 0.01f)
			{
				return true;
			}
			return false;
		...
		{
			Vector3 vector = CalculateBoundsOverlap(boundsA, boundsB);
		...
		{
			Vector3 vector = CalculatePerAxisOverlap(boundsB, boundsA);
			float num;
			switch (upDirection)
			{
		...
		{
			return new Vector3((input.x < min.x) ? min.x : ((input.x > max.x) ? max.x : input.x), (input.y < min.y) ? min.y : ((input.y > max.y) ? max.y : input.y), (input.z < min.z) ? min.z : ((input.z > max.z) ? max.z : input.z));
		}
