...
namespace DunGen.Graph
{
	...
	public class DungeonFlow : ScriptableObject, ISerializationCallbackReceiver
	{
		...
		public bool CanDoorwaysConnect(ProposedConnection connection)
		{
			...
			{
				TileConnectionRule.ConnectionResult connectionResult = TileConnectionRule.ConnectionResult.Passthrough;
				if (item.ConnectionDelegate != null)
				{
					connectionResult = item.ConnectionDelegate(connection);
				}
				else if (item.Delegate != null)
				{
					connectionResult = item.Delegate(connection.PreviousTile.PrefabTile, connection.NextTile.PrefabTile, connection.PreviousDoorway.DoorwayComponent, connection.NextDoorway.DoorwayComponent);
				}
			...
			if (DoorwaySocket.CanSocketsConnect(connection.PreviousDoorway.DoorwayComponent.Socket, connection.NextDoorway.DoorwayComponent.Socket))
			{
				return CanTilesConnect(connection.PreviousTile.PrefabTile, connection.NextTile.PrefabTile);
			}
			return false;
		}
